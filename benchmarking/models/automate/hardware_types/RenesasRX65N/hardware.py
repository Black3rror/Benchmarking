...
from benchmarking.models.automate.hardware_template import HardwareTemplate
from benchmarking.models.utils.utils import get_abs_path


config_file_path = "benchmarking/models/automate/hardware_types/RenesasRX65N/configs/hardware_config.yaml"
...
class Hardware(HardwareTemplate):
    def __init__(self, software_platform):
        ...
        self.cfg.project_name = "RenesasRX_{}".format(software_platform)


    def get_model_dir(self):
        return os.path.join(self.cfg.project_dir, "src/model")


    def build_project(self, clean=False):
        task = "-build" if not clean else "-cleanBuild"
        command = [
            self.cfg.e2studio_path,
    ...
    def upload_program(self):
        mot_path = os.path.join(self.cfg.project_dir, "HardwareDebug", os.path.basename(self.cfg.project_dir) + ".mot")
        mot_path = get_abs_path(mot_path)
        command = [
            self.cfg.rfp_path,
            "/silent",
            self.cfg.rfp_project_path,
            "/file", mot_path
        ]

        for _ in range(3):      # RFP is not reliable, so try 5 times
