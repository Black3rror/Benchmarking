...
try:    # in case requirements are not installed
    import benchmarking.models.automate.automate as automate
    import benchmarking.models.platforms.eAI_Translator.eAI_Translator_converter as eai_translator_converter
    import benchmarking.models.platforms.EI.EI_converter as ei_converter
    import benchmarking.models.platforms.TensorFlow.model_generator as tf_model_generator
    import benchmarking.models.platforms.TFLite.TFLite_converter as tflite_converter
    import benchmarking.models.platforms.TFLM.TFLM_converter as tflm_converter
    import benchmarking.models.platforms.Ekkono.model_generator as ekkono_model_generator   # keep it last, so it won't it won't be a backdoor for other errors
...
def main():
    ...
    ei_converter_config_path = "benchmarking/models/platforms/EI/configs/EI_converter_config.yaml"
    stm32_automate_config_path = "benchmarking/models/automate/hardware_types/NUCLEO-L4R5ZI/configs/hardware_config.yaml"
    renesas_automate_config_path = "benchmarking/models/automate/hardware_types/RenesasRX65N/configs/hardware_config.yaml"
    target_dir = "benchmarking/target_models"
    ...
    while not check_package_requirements(reqirements_file):
        q = _Question("Packages are missing. What do you want to do",
                      description="Please make sure that you are in the correct environment.\nIt's best to create a new Conda environment with 'python 3.11.7'. The command would be 'conda create -n benchmarking python=3.11.7'. Then activate the environment with 'conda activate benchmarking' and run the project again.",
    ...
    if "ekkono_sdk" in modules_requiremets:
        while not _check_ekkono_installation():
            ...
            q = _Question("Ekkono is not installed. Please provide the path to the wheel file",
                          description="Ekkono is not free. If you want to use it, you need to buy this product from https://www.ekkono.ai, download the its files and provide the wheel path here.\nFor example, put the files in 'benchmarking/models/platforms/Ekkono' and provide the path 'benchmarking/models/platforms/Ekkono/ekkono-sdk/primer/python/{distribution}/{python-version}/ekkono.primer-{name-suffix}.whl'")
