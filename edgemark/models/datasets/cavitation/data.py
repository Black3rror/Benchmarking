...
class DatasetSupervisor(DatasetSupervisorTemplate):
    def __init__(self, data_type, problem_type, window_size, test_sep_strategy, test_ratio, flat_features=True, normalize=True, random_seed=None):
        super().__init__()

        self.data_type = data_type
        self.problem_type = problem_type
        self.window_size = window_size
        self.test_sep_strategy = test_sep_strategy
        self.test_ratio = test_ratio
        self.flat_features = flat_features
        self.normalize = normalize
        self.random_seed = random_seed

        self.pumps = [(5, 3), (5, 18), (5, 36), (32, 3), (64, 2), (95, 2), (125, 2)]    # (flow, stages)
        self.dataset_root_dir = os.path.join(os.path.dirname(__file__), "data")
        self.pump_metadata_file_path = os.path.join(self.dataset_root_dir, "raw/CR {pump_flow}-{pump_stages} calculations.xlsx")
        self.pump_metadata_sheet_name = "NPSH points"
        self.pump_record_file_path = os.path.join(self.dataset_root_dir, "raw/CR{pump_flow}-{pump_stages}/{record_file_name}")

        (self.train_m, self.train_x, self.train_y), (self.test_m, self.test_x, self.test_y) = self._load_dataset(data_type, problem_type, window_size, test_sep_strategy, test_ratio, flat_features, normalize, random_seed)
        ...
        if problem_type == "regression":
            ...
            self.metrics = ["mse", "mae", regression_accuracy]
    ...
    def load_dataset(self, data_type, problem_type, window_size, test_sep_strategy, test_ratio, flat_features=True, normalize=True, random_seed=None):
        """
        Loads the dataset.
        ...
        """
        (_, train_x, train_y), (_, test_x, test_y) = self._load_dataset(data_type, problem_type, window_size, test_sep_strategy, test_ratio, flat_features, normalize, random_seed)

        return (train_x, train_y), (test_x, test_y)


    def _load_dataset(self, data_type, problem_type, window_size, test_sep_strategy, test_ratio, flat_features=True, normalize=True, random_seed=None):
        cached = False
        if os.path.exists(os.path.join(self.dataset_root_dir, "processed/cache_info.yaml")):
            cache_info = OmegaConf.load(os.path.join(self.dataset_root_dir, "processed/cache_info.yaml"))
        ...
        if not cached:
            ...
            dataset_x = []
            dataset_y = []
            for i in tqdm(range(len(self.pumps)), desc="Loading data", colour='green', leave=False):
                pump = self.pumps[i]

                pump_metadata = self._read_pump_metadata(pump[0], pump[1], exclude_partial_data=True)
                ...
                for j in range(len(pump_records_filenames)):
                    pump_record = self._read_pump_record(pump[0], pump[1], pump_records_filenames[j])
            ...
            # process the data
            dataset_x = self._convert_from_measurement_to_acceleration(dataset_x)
            ...
            if test_sep_strategy is None:
                ...
                rng = np.random.RandomState(random_seed)
                pumps_test_chosen = rng.choice(len(self.pumps), size=round(len(self.pumps)*test_ratio), replace=False)
                pumps_test = [self.pumps[i] for i in pumps_test_chosen]
                pumps_train = [i for i in self.pumps if i not in pumps_test]
            ...
            os.makedirs(os.path.join(self.dataset_root_dir, "processed"), exist_ok=True)
            OmegaConf.save(OmegaConf.create({"data_type": data_type, "problem_type": problem_type, "window_size": window_size_original, "test_sep_strategy": test_sep_strategy, "test_ratio": test_ratio, "flat_features": flat_features, "random_seed": random_seed}), os.path.join(self.dataset_root_dir, "processed/cache_info.yaml"))
            np.savez_compressed(os.path.join(self.dataset_root_dir, "processed/dataset.npz"), train_m=train_m, train_x=train_x, train_y=train_y, test_m=test_m, test_x=test_x, test_y=test_y)

        else:
            dataset = np.load(os.path.join(self.dataset_root_dir, "processed/dataset.npz"), allow_pickle=True)
    ...
    def _read_pump_metadata(self, pump_flow, pump_stages, exclude_partial_data=True):
        """
        ...
        pump_metadata_file_path = self.pump_metadata_file_path.format(pump_flow=pump_flow, pump_stages=pump_stages)
        pump_metadata = pd.read_excel(pump_metadata_file_path, sheet_name=self.pump_metadata_sheet_name, header=None)
    ...
    def _read_pump_record(self, pump_flow, pump_stages, record_file_name):
        pump_record_file_path = self.pump_record_file_path.format(pump_flow=pump_flow, pump_stages=pump_stages, record_file_name=record_file_name)
